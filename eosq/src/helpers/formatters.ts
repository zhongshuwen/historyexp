...
export function secondsToTime(seconds: number) {
  ...
  const momentDurationFormat: any = moment.duration(seconds, "seconds")
  return momentDurationFormat.format("Y [years] M [months] D [days] [+] h:mm:ss")
}

export function explodeJson(jsonOrString: Record<string, any> | string): string {
  if (!jsonOrString) {
    return ""
  }
  ...
  const json: Record<string, any> = jsonOrString
