...
func (t *tableScope) payerActions(primKey string, abi *zsw.ABI, logger *zap.Logger) (excl *string, preAct []*zsw.Action, postAct []*zsw.Action, err error) {
...
func (t *tableScope) rowToActions(abi *zsw.ABI, primKey string, logger *zap.Logger) (out []*zsw.Action, err error) {
	...
	out = append(out, newInjectAct(t.account, t.table, t.scope, AN(row.Payer), zsw.Name(row.Key), data))
	...
	if traceEnable {
		logger.Debug("contract row inject",
			...
			zap.Stringer("bytes", zsw.HexBytes(data)),
		)
	}
...
type tableRow struct {
	...
	DataHex          zsw.HexBytes      `json:"hex_data,omitempty"`
...
func (t *tableRow) encode(abi *zsw.ABI, table zsw.TableName) ([]byte, error) {
	if len(t.DataHex) > 0 {
...
func (t *tableRow) indexesToAction(account zsw.AccountName, table zsw.TableName, scope zsw.ScopeName) (out []*zsw.Action, err error) {
	for id, index := range t.SecondaryIndexes {

		indexTable := mustCreateIndexTable(string(table), uint64(id))
		action, err := index.idxToAction(account, TN(indexTable), scope, zsw.Name(t.Key))
		if err != nil {
...
func (s *secondaryIndex) idxToAction(account zsw.AccountName, tableName zsw.TableName, scope zsw.ScopeName, primKey zsw.Name) (*zsw.Action, error) {
	switch s.Kind {
