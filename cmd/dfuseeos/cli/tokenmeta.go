...
import (
	...
	tokenmetaApp "github.com/zhongshuwen/historyexp/tokenmeta/app/tokenmeta"
...
func init() {
	launcher.RegisterApp(&launcher.AppDef{
		...
		MetricsID:   "tokenmeta",
		Logger:      launcher.NewLoggingDef("github.com/zhongshuwen/historyexp/tokenmeta.*", nil),
		RegisterFlags: func(cmd *cobra.Command) error {
			...
			cmd.Flags().String("tokenmeta-abis-base-url", "{zswlishi-data-dir}/storage/abicache", "cached ABIS base URL")
			cmd.Flags().String("tokenmeta-abis-file-name", "abi-cache.json.zst", "cached ABIS filename")
			cmd.Flags().String("tokenmeta-cache-file", "{zswlishi-data-dir}/tokenmeta/token-cache.gob", "Path to GOB file containing tokenmeta cache. will try to Load and Save to that cache file")
