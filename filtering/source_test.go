	...
	tests := []struct {
	...
	}{
		{
			...
			ct.Block(t, "00000001aa",
				ct.TrxTrace(t, ct.ActionTrace(t, "zswhq:zswhq:newaccount")),
			...
			ct.Block(t, "00000001aa", ct.FilteredBlock{
				...
				ct.TrxTrace(t, ct.ActionTrace(t, "zswhq:zswhq:newaccount", ct.ActionMatched)),
			),
		},
	}
...
func TestFilteringTwice(t *testing.T) {
	tests := []struct {
		...
		{
			"include filter works on actions in filtered block",
			getFilters(`action == "transfer"`, "", ""),
			getFilters(`receiver == "spamcoin"`, "", ""),
			ct.Block(t, "00000001aa",
				ct.TrxTrace(t,
					ct.ActionTrace(t, "zswhq:zswhq:newaccount"),
			...
			ct.Block(t, "00000001aa", ct.FilteredBlock{
				Include:         `action == "transfer"`,
				Exclude:         "",
				UnfilteredStats: ct.Counts{1, 3, 3},
				FilteredStats:   ct.Counts{1, 2, 2},
			},
				ct.TrxTrace(t,
					ct.ActionTrace(t, "zswhq:zswhq:newaccount"),
					ct.ActionTrace(t, "spamcoin:spamcoin:transfer", ct.ActionMatched),
